...
def crawl_word(key_word, important, json_file):
    ...
    for btn in range(len(next_btn))[1:]:
        # 중학교 명단 리스트
        market_list = driver.find_elements(By.CSS_SELECTOR, 'li.VLTHu.OW9LQ')

        # 중학교 이름 리스트
        market_name = driver.find_elements(By.CSS_SELECTOR, 'span.place_bluelink.YwYLL')
                    ...
                    try:
                        ...
                        if rescode == 200:
                            ...
                            if response_body['addresses'] == []:
                                ...
                                middle_dict[important].append(dict_temp)
                        else:
        ...
        if market_name[-1]: # 마지막 중학교 경우 다음버튼 클릭
            next_btn[-1].click()
            sleep(2)
        else:
            print('페이지 인식 못함')
    ...
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(middle_dict, f, indent=4, ensure_ascii=False)

 # 검색어
key_word2 = '서울중학교'
important2 = '중학교'
crawl_word(key_word2, important2, 'E:\\codeitpython\\middle.json')
