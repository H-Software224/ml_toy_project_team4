...
def crawl_word(key_word, important, json_file):
    ...
    for btn in range(len(next_btn))[1:]:
        # 초등학교 항목리스트
        elementary_list = driver.find_elements(By.CSS_SELECTOR, 'li.VLTHu.OW9LQ')

        # 초등학교 이름 리스트
        elementary_name = driver.find_elements(By.CSS_SELECTOR, 'span.place_bluelink.YwYLL')

        for data in range(len(elementary_list)):
            ...
            try:
                ...
                # (3) 시장명 가져오기
                shop_name = elementary_name[data].text
                    ...
                    try:
                        ...
                        if rescode == 200:
                            ...
                            if response_body['addresses'] == []:
                                ...
                                elementary_dict[important].append(dict_temp)
                        else:
        ...
        if elementary_name[-1]: # 마지막 주차장일 경우 다음버튼 클릭
            next_btn[-1].click()
            sleep(2)
        else:
            print('페이지 인식 못함')
    ...
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(elementary_dict, f, indent=4, ensure_ascii=False)

 # 검색어
key_word2 = '서울초등학교'
important2 = '초등학교'
crawl_word(key_word2, important2, 'E:\\codeitpython\\elementary.json')
